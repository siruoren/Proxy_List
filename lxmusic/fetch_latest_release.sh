...
if [ -n "${GITHUB_REPOSITORY:-}" ]; then
    ...
    RAW_BASE="https://wget.la/https://raw.githubusercontent.com/${GITHUB_REPOSITORY}/$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo main)"
else
    ...
    if [ -n "$ORIGIN_URL" ]; then
        ...
        RAW_BASE="https://wget.la/https://raw.githubusercontent.com/${REPO_SLUG}/${BRANCH}"
    fi
fi
